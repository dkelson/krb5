...
def set_enforcing_mode(realm, enforcing):
    ...
    realm.stop_kdc()
    realm_env = realm.special_env('enforce_config', True,
                                  kdc_conf=mode_kdc_conf)
    ...
    with open(kdc_log, 'r') as f:
        log_content = f.read()
        expected_init_msg = f"xrealmauthz cross-realm authorization module loaded (enforcing mode: {expected_msg},"
        if expected_init_msg not in log_content:
            fail(f'Expected module init message with enforcing mode {expected_msg}')
...
def verify_allowed_realms(r1, r2, r3, enforcing=None):
    ...
    r1.stop_kdc()
    realm_env = r1.special_env('multi_allowed', True,
                               kdc_conf=multi_realm_conf)
    r1.start_kdc(env=realm_env)
...
realm1_kdc_conf = {'plugins': {'kdcpolicy': {'module':
                                             'xrealmauthz:' + plugin_path}}}
